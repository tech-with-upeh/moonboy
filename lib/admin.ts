...
export async function getAllComments(): Promise<CommentWithPost[]> {
  const rows = await db.comment.findMany({ include: { post: { select: { slug: true, title: true } } }, orderBy: { createdAt: "desc" } });
  return rows.map((comment) => ({ id: comment.id, name: comment.name, initials: comment.name.trim().split(/\s+/).slice(0, 2).map((part) => part[0]?.toUpperCase() ?? "").join(""), date: comment.createdAt.toISOString(), body: comment.body, status: comment.status, slug: comment.post.slug, postTitle: comment.post.title }));
}
export async function getTotals() {
  const withStats = await getPostsWithStats(); const totalPosts = withStats.length; const totalLikes = withStats.reduce((sum, p) => sum + p.likes, 0); const totalComments = withStats.reduce((sum, p) => sum + p.commentCount, 0); const avgReadTime = Math.round(withStats.reduce((sum, p) => sum + p.readTime, 0) / (totalPosts || 1));
